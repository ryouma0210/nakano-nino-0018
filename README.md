...
| ホーム | 「調教」「二ノ様の控室」「記録・交換メニュー」「管理・設定メニュー」への入口。契約後は専用会話を追加 |
| 調教 | 敗北部屋、準備部屋、本日の命令部屋、調教部屋、射精管理部屋、お仕置き部屋、契約部屋への入口 |
| 敗北部屋 | 契約者限定。上部に専用画像、下に拡大可能な敗北動画、日付単位の強制チェック、専用音声、同心ハート演出、調教日記への保存。拡大動画の閉じるボタンは画面下に固定。廊下の敗北部屋ボタンだけ赤背景、赤枠、白文字 |
| 準備部屋 | 必須・任意項目の確認、準備用動画と音声、日付単位の保存。拡大動画の閉じるボタンは画面下に固定 |
| 本日の命令部屋 | 日付単位でランダム命令を固定。完了で1pt |
| 調教部屋 | デフォルト動画または調教用格納ファイル、難易度別リズムゲージ、コメント、完了秒数を保存。WEB版でも動画ルールに従って次ファイルへ進む。ハートマーカーはWEBでも崩れにくい図形表示。習慣登録UIは表示しない。1日初回5pt |
| 射精管理部屋 | 貞操帯あり／なしを選択。サイコロの目×3日で期間を決定し、日次指示と最終日指示を保存。1日完了10pt |
| お仕置き部屋 | デフォルト動画またはお仕置き用格納ファイル、時間指定、ランダム速度、指示コメント、ギブアップ、実施時間の保存。WEB版でも格納動画を再生。QマーカーはWEBでも崩れにくい図形表示 |
...
画面構成は、上部に敗北部屋用の二ノ画像、下にタップで拡大できる敗北動画、その下にチェック項目を配置します。拡大動画の閉じるボタンはWEB／Windowsでも押せるように画面下へ固定します。ハート動画は5秒ごとに約2秒だけ最前面へ表示し、タップ操作は邪魔しないようにしています。
