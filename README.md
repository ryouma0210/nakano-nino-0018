...
| 敗北部屋 | 契約者限定。上部に専用画像、下に拡大可能な敗北動画、日付単位の強制チェック、専用音声、同心ハート演出、調教日記への保存。ボタンはピンク背景、赤枠、赤文字 |
| 準備部屋 | 必須・任意項目の確認、準備用動画と音声、日付単位の保存 |
| 本日の命令部屋 | 日付単位でランダム命令を固定。完了で1pt |
| 調教部屋 | デフォルト動画または調教用格納ファイル、難易度別リズムゲージ、コメント、完了秒数を保存。習慣登録UIは表示しない。1日初回5pt |
...
| 二ノ様の控室 | 画像／動画表示の専用画面。セリフ表示、画像／動画切り替え、衣装の着せ替えが可能 |
...
画面構成は、上部に敗北部屋用の二ノ画像、下にタップで拡大できる敗北動画、その下にチェック項目を配置します。ハート動画は5秒ごとに約2秒だけ最前面へ表示し、タップ操作は邪魔しないようにしています。
