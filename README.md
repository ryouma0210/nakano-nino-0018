...
| 画面 | 現在の内容 |
|---|---|
| スタート | 「始める」「設定（サウンドのみ）」「ゲーム終了」、アプリバージョンを表示。起動時に今月のログインボーナス受取状況をスタンプ式で表示し、未受取なら「受け取る」でPt付与とスタンプ反映、受取済みなら「閉じる」のみ表示。本日の獲得Ptも確認可能 |
...
| 管理・設定メニュー | ループ音声、ファイル格納、マイページ、設定、外部リンクへの入口 |
...
| 奴隷契約書 ※本物 | 契約者名、契約日、解約日（月単位）を入力し、契約書を表示。契約者名なしでも空欄の契約書をPDF出力可能。APPでは共有ではなく端末内へ保存、WEBでは印刷画面からPDF保存。設定の「契約書・契約ルール」初期化で入力内容を削除 |
