...
| 画面 | 現在の内容 |
|---|---|
| スタート | 「始める」「設定（サウンドのみ）」「ゲーム終了」、アプリバージョンを表示。起動時に今月のログインボーナス受取状況をスタンプ式で表示し、本日の獲得Ptも確認可能 |
...
| 本日の記録 | ログインボーナスの確認・受取と、本日の調教回数、管理日数、獲得ポイント、命令完了、お仕置き時間を確認。起動時モーダルでも今月のログインスタンプと本日の獲得Ptを確認可能。上部の二ノ会話と評価セリフは表示しない |
